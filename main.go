...
func main() {
	...
	if err := cfg.Get("GRPCServer").Scan(&sconfig); err != nil {
		log.Error("get grpc server config failed", zap.Error(err))
	}
	log.Sugar().Debugf("grpc server config, %+v", sconfig)
...
func runHTTPServer(cfg ServerConfig) {
	...
	if err := gt.RegisterGreeterGwFromEndpoint(ctx, mux, cfg.GRPCListenAddress, opts); err != nil {
